...
import type { UserProfile, StadiumState, ContextRecommendation } from "@/lib/types";
import { generateStadiumState } from "@/lib/data/mockStadiumData";

const mockProfile: UserProfile = {
  role: "fan",
  currentZone: "north-lower",
  accessibilityNeeds: { wheelchairAccess: false, visualImpairment: false, hearingImpairment: false },
};
...
describe("aiService Offline Gateway", () => {
  ...
  it("should use deterministic fallback when API key is missing", async () => {
    ...
    const response = await generateChatResponse("where is food?", mockProfile, mockState);
  ...
  it("should fallback when message asks about weather", async () => {
    ...
    const response = await generateChatResponse("what is the weather?", mockProfile, mockState);
    expect(response.reply).toMatch(/⛅/);
  });
  ...
  it("should fallback when message asks about score", async () => {
    ...
    const response = await generateChatResponse("what's the score?", mockProfile, mockState);
    expect(response.reply).toMatch(/⚽/);
  });
