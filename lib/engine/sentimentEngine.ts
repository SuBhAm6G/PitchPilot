...
export function computeSentiment(stadiumState: StadiumState): SentimentScore {
  ...
  const criticalIncidents = stadiumState.incidents.filter(inc => inc.severity >= 3 && inc.status !== "resolved").length;
  ...
  if (stadiumState.matchState && stadiumState.matchState.events) {
    const recentGoals = stadiumState.matchState.events.filter(
      e => e.type === "goal" && stadiumState.matchState.currentMinute - e.minute <= 10
    ).length;
    if (recentGoals > 0) {
      score += 1;
    }
  ...
  switch (score) {
    case 5: return { level: 5, label: "Electric", emoji: "⚡" };
    case 4: return { level: 4, label: "Excited", emoji: "🔥" };
    case 3: return { level: 3, label: "Engaged", emoji: "👀" };
    case 2: return { level: 2, label: "Mild", emoji: "😐" };
    case 1:
    default: return { level: 1, label: "Flat / Tense", emoji: "📉" };
  }
}
