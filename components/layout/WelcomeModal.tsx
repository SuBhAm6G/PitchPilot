...
interface WelcomeModalProps {
  ...
  readonly onComplete: (role: UserRole, zone: ZoneId, seat: string) => void;
}

export default function WelcomeModal({ stadiumData, onComplete }: WelcomeModalProps) {
  ...
  async function handleGenerateBriefing(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          message: `I have just arrived. My role is ${role}, zone is ${zone}, and seat is ${seat || "unassigned"}. Please provide a comprehensive, 3-sentence welcome briefing with all necessary information at one go including weather, nearest food, and match status. Do not ask follow up questions.`,
      ...
      if (response.ok) {
        ...
        setBriefing("Welcome to MetLife Stadium! Please proceed to your zone for more details.");
      }
    } catch {
      setBriefing("Welcome to MetLife Stadium! Please proceed to your zone for more details.");
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="p-6">
          <h2 className="text-2xl font-bold text-white mb-2">Welcome to PitchPilot</h2>
          <p className="text-slate-400 text-sm mb-6">Let&apos;s set up your match-day experience.</p>
          ...
          {!briefing ? (
            <form onSubmit={handleGenerateBriefing} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">Current Zone</label>
                <select
                  value={zone}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">Seat Number (Optional)</label>
                <input
                  type="text"
                  value={seat}
            ...
            <div className="space-y-6 animate-in slide-in-from-right duration-300">
              <div className="bg-indigo-900/30 border border-indigo-500/30 rounded-lg p-4">
                ...
                <p className="text-slate-200 text-sm leading-relaxed">{briefing}</p>
              </div>

              <button
                onClick={() => onComplete(role, zone, seat)}
