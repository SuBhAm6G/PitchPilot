...
const MatchTimeline = memo(function MatchTimeline({ matchState }: MatchTimelineProps) {
    ...
    <Card as="section" aria-label="Match Timeline">
      <div className="flex items-center justify-between border-b border-slate-700/50 pb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xl font-bold">{matchState.homeScore} - {matchState.awayScore}</span>
      ...
      <div className="mt-4 flex flex-col gap-3">
        {recentEvents.length === 0 ? (
          ...
          recentEvents.map((event) => (
            <div key={event.id} className="flex items-start gap-3 rounded-lg bg-slate-800/30 p-3">
              ...
              <div>
                <p className="text-sm font-medium">
                  {event.minute}&apos; - {MATCH_EVENT_LABELS[event.type] || event.type}
                </p>
