    ...
    <Card as="section">
      ...
      <div className="flex flex-col gap-4">
        {venueTypes.map((type) => {
          ...
          const avgWait = typeVenues.length > 0
            ? Math.round(typeVenues.reduce((sum, v) => sum + v.estimatedWaitMinutes, 0) / typeVenues.length)
            : 0;
          const meta = VENUE_LABELS[type];

          return (
            <div key={type} className="flex items-center justify-between rounded-xl bg-slate-800/30 px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{typeVenues.length} open</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-lg font-bold text-white">{avgWait}m</span>
                <Badge label={getWaitVariant(avgWait)} variant={getWaitVariant(avgWait)} />
              </div>
            </div>
          );
        })}
