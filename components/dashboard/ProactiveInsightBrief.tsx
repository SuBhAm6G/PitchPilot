...
const ProactiveInsightBrief = memo(function ProactiveInsightBrief({ stadiumState }: ProactiveInsightBriefProps) {
  const [insight, setInsight] = useState<string>("Analyzing stadium operations...");
  const [isLoading, setIsLoading] = useState(true);

  // Stable dependency key — only re-fetch when match phase or timestamp actually changes
  const stateFingerprint = useMemo(
    () => (stadiumState ? `${stadiumState.matchPhase}-${stadiumState.lastUpdated}` : null),
    [stadiumState]
  );
  ...
  useEffect(() => {
    ...
    async function fetchInsight() {
      try {
        ...
        if (isMounted) setInsight("Unable to generate AI briefing at this time.");
      } finally {
  ...
  return (
    <Card as="section" className="col-span-full border-indigo-500/30 bg-indigo-900/10">
          ...
          <div className="mt-2 text-sm text-slate-300 leading-relaxed min-h-[2.5rem]">
            {isLoading ? (
              <div className="flex items-center gap-2">
                <span className="h-4 w-4 animate-spin rounded-full border-2 border-indigo-500/30 border-t-indigo-500" aria-hidden="true" />
                <span className="text-slate-400 animate-pulse">Generating insights...</span>
              </div>
            ) : (
