      ...
      <div className="mt-4 flex flex-col gap-4">
        {myIncidents.length > 0 && myIncidents[0] ? (
          <div className="rounded-lg border border-indigo-500/20 bg-slate-900/50 p-3">
            <h3 className="text-xs font-semibold text-slate-400">Your Current Task</h3>
            <p className="mt-1 font-medium text-indigo-200">{myIncidents[0].title}</p>
            <p className="mt-1 text-xs text-slate-300">Location: {myIncidents[0].zoneId}</p>
          </div>
          ...
          <p className="text-sm text-slate-400">No active assignments. Standby in your current zone.</p>
        )}
      </div>
    </Card>
  );
